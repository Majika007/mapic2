...
import exifread
        ...
        self.toggleBtn = QPushButton("☯")   # Yin-Yang symbol looks nice
        self.toggleBtn.setToolTip("Toggle Dark/Light theme")
        self.toggleBtn.setFixedWidth(50)    # keep small
        self.toggleBtn.setFixedHeight(30)
        self.toggleBtn.clicked.connect(self.toggle_theme)
...
class ImageViewer(QWidget):
    ...
    def show_image(self, fname):
        ...
        <span class="key">✅ Prompt:</span> {result[0]}<br>
        <span class="key">🚫 Negative Prompt:</span> {result[1]}<br>
        <span class="key">📦 Checkpoint:</span> {result[2]}<br>
        <span class="key">🔁 Sampler:</span> {result[3]} &nbsp; 
        <span class="key">📈 Scheduler:</span> {result[4]} &nbsp;
        <span class="key">📏 Steps:</span> {result[5]}<br>
        <span class="key">🎯 CFG Scale:</span> {result[6]} &nbsp;
        <span class="key">🎲 Seed:</span> {result[7]} &nbsp;
    ...
    def save_meta(self):
        ...
        with open(txt_name, "w", encoding="utf-8") as f:
            ...
            f.write(f"🚫 Negative Prompt: {result[1]}\n")
            f.write(f"📦 Checkpoint: {result[2]}\n")
            f.write(f"🔁 Sampler: {result[3]}\n")
            f.write(f"📈 Scheduler: {result[4]}\n")
            f.write(f"📏 Steps: {result[5]}\n")
            f.write(f"🎯 CFG Scale: {result[6]}\n")
            f.write(f"🎲 Seed: {result[7]}\n")
            f.write(f"🌀 Denoise: {result[8]}\n")
            f.write(f"🧠 VAE: {result[9]}\n")
            f.write(f"✨ LoRA: {result[10]}\n")
