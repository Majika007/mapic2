...
def extract_prompts_jpg(image_path):
    """
    Robosztus A1111/Comfy-style JPEG UserComment olvasó.
    Visszatérési formátum kompatibilis a PNG változattal:
    (pos, neg, ckpt, sampler, scheduler, step, cfg, seed, denoise, vae, lora)
    """
    try:
        raw = None

        # 1) Próbáljuk exifread-del
        try:
            ...
            if uc:
                ...
                if isinstance(raw, bytes):
                    # próbáljuk utf-8-cal, ha nem megy, latin1 fallback
                    try:
        ...
        # 2) Fallback: exiftool JSON lekérdezés (megbízható)
        if not raw or str(raw).strip() == "":
            try:
                p = subprocess.run(['exiftool', '-j', '-G1', '-a', image_path],
                                   stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
                if p.returncode == 0 and p.stdout:
                    arr = json.loads(p.stdout)
                    if isinstance(arr, list) and len(arr) > 0:
                        obj = arr[0]
                        # keressük a UserComment kulcsot (címkéket exiftool formátumban)
                        uc_key = None
                        for k in obj.keys():
                            if k.lower().endswith(':usercomment') or k.lower() == 'usercomment':
                                uc_key = k
                                break
                        if uc_key:
                            raw = obj.get(uc_key)
        ...
        # ha körbe vannak idézőjelek, távolítsuk el, és unescape-eljük
        ...
        # próbáljuk meg betölteni JSON-ként többféleképpen
        data = None
        try:
            data = json.loads(s)
        except json.JSONDecodeError:
            # próbáljuk meg a legelső { ... } részt kinyerni
            first = s.find('{')
            last = s.rfind('}')
            if first != -1 and last != -1 and last > first:
                ...
                try:
                    ...
                    # próbáljuk meg az escaped idézőjeleket visszacserélni
                    t = s.replace('\\"', '"')
                    try:
                        data = json.loads(t)
                    except Exception:
                        data = None
        if data is None:
            # végső próbálkozás: ha s maga JSON-szerű, próbáljuk unicode unescape után
            try:
        ...
        if data is None:
            # nem sikerült JSON-ná tenni
            return ("N/A", "N/A", "-", "-", "-", "-", "-", "-", "-", "-", "-")

        # --- Promptok gyűjtése: keress minden "text" mezőt sorrendben (rekurzív) ---
        texts = []
        def collect_texts(obj):
            if isinstance(obj, dict):
                for k, v in obj.items():
                    # ha itt 'inputs' van és benne 'text', ezt vegyük
                    if isinstance(v, dict) and 'inputs' in v and isinstance(v['inputs'], dict) and 'text' in v['inputs']:
                        t = v['inputs']['text']
                        texts.append(" ".join(str(t).replace('\r',' ').replace('\n',' ').split()))
                    else:
                        collect_texts(v)
        ...
        # --- extraMetadata feldolgozás (ha van) ---
        step = cfg = sampler = seed = "-"
        extra = data.get('extraMetadata') if isinstance(data, dict) else None
        if extra:
            try:
                if isinstance(extra, str):
                    # extra gyakran beágyazott JSON-string, dekódoljuk
                    try:
                        em = json.loads(extra)
                    except json.JSONDecodeError:
                        em = json.loads(codecs.decode(extra, 'unicode_escape'))
                elif isinstance(extra, dict):
                    em = extra
                else:
                    em = None
                if em:
                    step = em.get('steps', em.get('step', '-'))
                    cfg  = em.get('cfgScale', em.get('cfg', '-'))
                    sampler = em.get('sampler', '-')
                    seed = em.get('seed', '-')
            except Exception:
                pass

        # --- Más mezők, amik JPEG-ben ritkán vannak ---
        ckpt = "-"                # általában nincs közvetlen ckpt_name mező itt
        scheduler = "-"
        denoise = "-"
        vae = "-"
        lora = "-"
...
class ImageViewer(QWidget):
    ...
    def show_image(self, fname):
        ...
        <span class="key1">📦 Checkpoint:</span> {result[2]}<br>
        ...
        <span class="key3">🌀 Denoise:</span> {result[8]}<br>
        <span class="key4">🧠 VAE:</span> {result[9]}<br>
        <span class="key1">✨ LoRA:</span> {result[10]}<br>
        """
