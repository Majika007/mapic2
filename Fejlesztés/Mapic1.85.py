...
class ImageViewer(QWidget): 
    def __init__(self):
        ...
        self.thumb_cache = {}  # {filename: QPixmap}
        ...
        # külön container az image nézethez
        ...
        # Nagy kép nézet (a mostani splitter)
        self.viewer_widget = QWidget()
        self.viewer_layout = QVBoxLayout(self.viewer_widget)
        self.viewer_layout.addWidget(self.splitter)
        self.stack.addWidget(self.viewer_widget)

        # Thumbnail nézet (ScrollArea)
        ...
        # Kezdetben a nagy kép nézet legyen látható
        self.stack.setCurrentWidget(self.image_view_widget)
        self.image_label.mouseDoubleClickEvent = self.show_thumbnails
    ...
    def show_thumbnails(self, event=None):
        ...
        for i in reversed(range(self.thumb_grid.count())):
            item = self.thumb_grid.itemAt(i).widget()
            if item:
                item.deleteLater()
        ...
        for i, path in enumerate(self.image_files):
            thumb = QLabel()
            thumb.setPixmap(self.get_thumbnail(path))
            thumb.mousePressEvent = lambda e, idx=i: self.open_image_from_thumb(idx)
            self.thumb_grid.addWidget(thumb, i // 6, i % 6)
    ...
    def open_image_from_thumb(self, index):
        ...
        self.show_image(self.image_files[self.current_index])
        self.stack.setCurrentWidget(self.image_view_widget)  # vissza nagy nézetre
    ...
    def toggle_theme(self):
        ...
        # újratöltjük az aktuális képet a helyes stílussal
        if self.image_files:
            self.show_image(self.image_files[self.current_index])
