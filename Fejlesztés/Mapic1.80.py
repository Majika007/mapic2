...
#        self.thumbnail_cache_done = False
        ...
        # ---------- interactions ----------
        # double-click the large image -> open thumbnails
    ...
    def get_thumbnail(self, path):
        return self.thumb_cache.get(path, QPixmap(160, 120))
...
#            self.thumbnail_cache_done = True
    ...
    def open_folder(self):
        ...
        # clear old thumbs in layout
        if hasattr(self, "thumb_layout"):
            for i in reversed(range(self.thumb_layout.count())):
                widget = self.thumb_layout.itemAt(i).widget()
                if widget is not None:
                    widget.deleteLater()  # biztos, hogy eltűnik a layoutból is

        self.thumb_cache.clear()

        if self.image_files:
            self.show_image(self.image_files[self.current_index])
            # cache indítása, ne blokkolja a UI-t
