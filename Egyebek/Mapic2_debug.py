...
def extract_loras_from_usercomment(raw_uc):
    """
    Kinyeri a LoRA-kat a UserComment mezőből JSON formátumból.
    ...
    try:
        ...
        matches = re.findall(r'\{[^\}]*"type"\s*:\s*"lora"[^\}]*\}', raw_uc)
        for m in matches:
            try:
                d = json.loads(m)
                if d.get("type") == "lora":
...
def extract_loras(obj):
    """Kinyeri a LoRA-kat különböző formátumokból.
...
def extract_from_usercomment(raw_uc):
    ...
    try:
        ...
        # --- sima szöveges formátum ---
            ...
            debug_log("Sima szöveges formátum :", m)
            if len(m) > 1:
...
#            debug_log("- Simaszövegesformátum: - : ", vae)
...
#        debug_log("------ Subprocess------- ", result)
        if result.returncode == 0 and result.stdout.strip():
            data = json.loads(result.stdout)
#            debug_log(" Datatok ::::::::::::::  ", data)
            if data and "UserComment" in data[0]:
                raw_uc = data[0]["UserComment"]

#                debug_log("--- itt hívjuk meg a helper függvényt ---", ".")
                parsed = extract_from_usercomment(raw_uc)
#                debug_log("--- itt jön ne vissza ---", parsed)
                if parsed:
                    return parsed
...
#            debug_log("Metadata-Pos:-----  ", pos)
#            debug_log("Metadata-Neg:-----  ", neg)
#            debug_log("Metadata-Ckpt:----- ", ckpt)
...
#        debug_log("Visszatért mezők: ", pos, result.loras )
        ...
        <span class="key3">🎲 Seed:</span> {result.seed} &nbsp;&nbsp;&nbsp;&nbsp;
